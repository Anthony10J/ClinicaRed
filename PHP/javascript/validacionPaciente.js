...
const expresiones = {
  ...
  password:
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-z\d!@#$%^&*()_+]+$/, // Validar sin espacios una minúscula, una mayúscula, un número y un caracter especial.
  dni: /^(?!00)(?!.*0{5}$)[0-9]{1,15}$/,
  dniN: /^[0-9]{1,15}$/,
};
...
let validarInputOcupacion = (e, campo) => {
  ...
  estadoValidacion.estadoCV = funciones.validarCampoVacio(
    e.target,
    `${campo}`,
  ...
  estadoValidacion.estadoCV
    ? (estadoValidacion.estadoER = funciones.validarExpresionRegular(
        expresiones.ocupacion,
        e.target,
        `${campo}`,
  ...
  estadoValidacion.estadoER
    ? (estadoValidacion.estadoUE = funciones.validarEspacios(
        /\s\s/g,
        e.target,
        `${campo}`,
        "Debe limitarse a un espacio"
      ))
    : "";

  estadoValidacion.estadoUE
    ? (estadoValidacion.estadoMC = funciones.validarMismoCaracter(
        e.target,
        `${campo}`,
        "No debe colocar el mismo caracter +2 veces seguidas"
      ))
    : "";
  estadoValidacion.estadoMC
    ? (estadoValidacion.estadoCM = funciones.validarCampoMaximo(
        e.target,
        `${campo}`,
        "No cumple la cantidad mínima de caracteres"
      ))
    : "";
...
let validarInputDNI = (e) => {
  ...
  estadoValidacion.estadoER
    ? (estadoValidacion.estadoCC = funciones.validarCerosConsecutivos(
        expresiones.dni,
        e.target,
        "numero_de_documento",
        "No válido"
  ...
  estadoValidacion.estadoCC
    ? (estadoValidacion.estadoCM = funciones.validarCampoMinimo(
        e.target,
        "numero_de_documento",
        "No cumple la cantidad mínima de caracteres"
      ))
...
export const validarFechaNacimiento = () => {
  ...
  const mensajeFechaNacimiento = document.getElementById(
...
const formulario = document.getElementById("registerFormPaciente");
...
document
  ...
  .addEventListener("change", validarFechaNacimiento);
...
formulario_Registro.addEventListener("submit", function (e) {
  const error_Formulario_Registro = document.querySelectorAll(
    ".formulario__grupo-incorrecto"
  );
  const error_Formulario = document.querySelectorAll(
